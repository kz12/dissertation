...
\begin{tikzpicture}[node distance = 6cm, auto]
    ...
    \node [block, right of=load, node distance = 6cm](update) {Update View};
    \node [block, right of=drawLoading, node distance = 6cm] (view) {Displaying Tree};
...
\begin{tikzpicture}[node distance = 5.5cm, auto]
    ...
    \node [decision, right of=child1, node distance = 6cm] (test2) {$size > threshold$};
    \node [block, right of=test2, node distance = 6cm] (child2) {node = child2};
...
\begin{tikzpicture}[node distance = 5.5cm, auto]
    ...
    \node [decision, right of=children, node distance = 6cm] (threshold) {$richness < threshold$};
    \node [block, right of=threshold, node distance = 6cm] (build) {Build all children of mini root};
    ...
    \node [decision, right of=test, node distance = 6cm] (inRunning) {node in running list?};
    \node [block, below of=inRunning, node distance = 3.5cm] (add2) {add node to non initialization list}; 
    \node [block, right of=leaf, node distance = 6cm] (end2) {end};
...
\begin{tikzpicture}[node distance = 5.5cm, auto]
    ...
    \node [block, right of=recalculation, node distance = 6cm] (poss) {Calculate Possibility};
    \node [block, right of=poss, node distance = 6cm] (sort) {Sort running list};
    \node [decision, below of=sort, node distance = 2.7cm] (space) {$memory < 10 Mb$};
    \node [block, left of=space, node distance = 6cm] (remove) {Remove 1st node \& its children from running list};    
    %Draw edges
    \path [line] (recalculation) -- (poss);
    \path [line] (poss) -- (sort);
    \path [line] (sort) -- (space);
	\draw [->] (7.7,-2.4) -- (10.8,-2.4);
	\draw [->] (10.8,-3.0) -- (7.7,-3.0);
\end{tikzpicture}
...
In order to reduce the response time, the extra finger move messages should be discarded. Another thread and message queue is used to solve this problem. When a user triggers a set of move actions, a set of recalculation messages will be sent from the main UI thread to the calculation thread. In the calculation thread, when it catches a recalculation message while there are other recalculation messages in the message queue, then the current recalculation message will be discarded. Otherwise, the recalculation method will be called. Hence, when there are several recalculation messages being sent, only the last one will be processed. When the recalculation is over, the calculation thread will send message to invalidate the view. 
 ...
 \begin{tikzpicture}[node distance = 6cm, auto]
    ...
    \node [block, right of=looper, node distance = 6cm] (catcher) {Catch next message};
    \node [decision, below of=handler, node distance = 3.5cm] (decision) {The only recalc message?};
    \node [block, left of=decision, node distance = 6cm] (recalculation) {Recalculation};
    %Draw edges
    \path [line] (view) -- (operation);
...
This is one cycle of allocating new nodes during idle time. When the cycle ends, the thread will send another idle calculation message. If the thread does not have recalculation message and there are enough space, then the cycle will be gone through again and one more idle calculation message will be generated. Otherwise, the idle calculation will terminate and be reactive after next user interaction.
 ...
 \begin{tikzpicture}[node distance = 5.5cm, auto]
    ...
    \node [decision, right of=term1, node distance=6cm] (test) {Recalculation in Message Queue?};
    \node [decision, below of=test, node distance = 4cm] (test2) {Enough memory?};
    \node [block, right of=test2, node distance = 6cm] (sort) {Sort non initialization list};
...
\begin{tikzpicture}[node distance = 4cm, auto]
    ...
    \node [block, right of=operation, node distance = 6.5cm] (update) {Update View};
    ...
    \node [block, left of=test, node distance = 6.5cm] (bitmap) {Translate and Scale bitmap};
    \node [block, below of=test] (redraw) {Draw Using Node Object};
    \node [block, left of=redraw, node distance = 6.5cm] (store) {Cache New Bitmap};
...
\begin{tikzpicture}[node distance = 4cm, auto]
	...
	\node [block, right of=counter, node distance =6cm] (bitmap) {Drawing bitmap};
	\node [decision, below of=bitmap, node distance = 4cm](recalculation){During recalculation};
	\node [block, right of=recalculation, node distance = 6cm] (redrawn) {Drawing nodes};
    %Draw edges
...
\begin{tikzpicture}[node distance = 4cm, auto]
    % Place nodes
    \node [block] (ends) {User interaction ends};
	\node [block, below of=test, node distance = 4cm](reset){duringInteraction = false};
	\node [block, below of=interaction, node distance = 4cm](redraw){Redraw};
	\node [block, right of=ends, node distance =10cm] (end2) {Recalculation ends};
	\node [block, below of=end2, node distance = 4cm](reset2){During recalculation =false};
    %Draw edges
     \path [line] (ends) -- (reset);
    \path [line] (reset) -- (redraw);
    \path [line] (end2) -- (reset2);
	\draw [-] (8.3,-4) -- (5.8,-4);
	\draw [-] (5.8,-4) -- (5.8,-2);
	\draw [-]  (5.8,-4) -- (5.8,-6);
	\draw [->] (5.8,-6) -- (0,-6) node[red,above,near start ] {Extra Drawing};
	\draw [->] (5.8,-2) -- (0,-2) node[red, above, near start] {Extra Drawing};
	\draw [-] (5.8, -6) -- (5.8, -10);
	\draw [->] (0, -8.7) -- (0,-12);
	\draw [->](5.8,-10) -- (0,-10) node[red, above, near start] {No Extra Drawing};
\end{tikzpicture}
\caption{Causes of extra drawing after user interaction}
\end{figure}

After users end interaction, only if the view updates itself with bitmap before the recalculation is done, there will be no extra drawing message. If the calculation thread ends calculating before the view updates, both the during recalculation thread and during interaction thread will be set false before invalidate message get executed. Hence, drawing nodes will be executed more than one time.

The solution to the above issue is to set a flag when actual drawing is being executed. This flag will be reset when user interacts with the view. If the flag is set, then when the view gets updated, the view will use bitmap to draw, otherwise, it will test other conditions and possibly do actual drawing. Hence, after user interaction, the actual drawing could only be called once before next interaction.

\begin{figure}[H]
\begin{tikzpicture}[node distance = 4cm, auto]
    % Place nodes
    \node [block] (ends) {User interaction ends};
	\node [decision, right of=ends, node distance = 6cm](test){flag equals 1?};
	\node [block, above of=test, node distance = 4cm](bitmap){Draw bitmap};
	\node [block, below of=test, node distance =4cm] (nodes) {Draw nodes};
	\node [block, below of=nodes, node distance = 4cm](set) {flag = 1};
    %Draw edges
     \path [line] (ends) -- (test);
    \path [line] (test) -- node[red]{Yes} (bitmap);
    \path [line] (test) -- node[red]{No}(nodes);
    \path [line] (nodes) -- (set);
	\draw [->] (12,0) -- (7.5,0) node[red,above, near start]{Invalidate Message};

\end{tikzpicture}
\caption{Solve extra drawing by setting a new flag}
\end{figure}
...
\begin{tikzpicture}[node distance = 4cm, auto]
    % Place nodes
    \node [block] (ends) {User interaction ends};
	\node [block, below of=ends, node distance = 4cm](high){Set high threshold};
	\node [block, below of=high, node distance = 4cm](recalc){Recalculation};
	\node [decision, right of=ends, node distance = 11cm](test) {Is user interacting?};
	\node [block, below of=test, node distance =4cm] (low) {Set low threshold};
	\node [block, below of = recalc] (update) {Update view};
    %Draw edges
     \path [line] (ends) -- (high);
    \path [line] (high) -- (recalc);
    \path [line] (ends) -- node[red]{Send Message 1 second later}(test);
    \path [line] (test) -- node[red]{No}(low);
    \path [line] (recalc) -- (update);
	\path [line] (low) |- (recalc);
\end{tikzpicture}
